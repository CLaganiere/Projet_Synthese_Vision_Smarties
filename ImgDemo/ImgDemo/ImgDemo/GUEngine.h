...
class GUEngine : public QMainWindow
{
	Q_OBJECT

public:
	GUEngine(QWidget *parent = Q_NULLPTR);

private:
	Ui::ImgDemoClass ui;

	QPushButton *mOptionWebCam, *mOptionDoc, *mOptionRetour, *mOptionDebug;
	QPushButton *mOptionLightCard, *mOptionProcImg;
	QPushButton *mConnectButton, *mDisconnectButton, *mCaptureOneButton, *mCaptureContinuouslyButton, *mOpionSave;
	QPushButton *mCaptureBckgButton, *mProcessLightCard, *mOpionSaveLightCard, *mOpionSaveDebug, *mOpionSaveDataDebug;
	QPushButton *mChooseImage,*mLoadPredefinedImage;
	QCheckBox *checkBtnBlue, *checkBtnGreen, *checkBtnBrown,
				*checkBtnOrange, *checkBtnPink, *checkBtnPurple,
				*checkBtnRed, *checkBtnYellow;
	QButtonGroup *buttonColorGroup;

	QNIntervalScrollBar *hsvIntervals;
	QProgressBar * progressBar, * progressBarDebugPrep;

	QLabel * AnswerLabel;

	ProcessImageEngine mProcessEngine;

	//QSimpleImageGrabber mSimpleImageGrabber;
	WebCamGrabber mWebCamGrabber;
	FileImageGrabber mFileImageGrabber;
	FileImageSaver mFileImageSaver;
	
	QSimpleImageViewer * mInputImage;
	QSimpleImageViewer * mProcessedImage;

	int menu{ 0 };
	int processCountLightCard{};
	int processCount{};
	int debugPrepState{};
	int currentColorDebug{};

	QTimer mTimer;
	bool mCapturingContinuously;
	bool mFirstProcess{ true };
	bool mFirstDebug{ true };
	//bool mImgSaver{ false };

private slots:
	void modificationMenu();
	void principalMenuChoice();
	void lightCardChoice();
	void processImageChoice();
	void webCamChoice();
	void docCamChoice();
	void debugChoice();
	void connectCamera();
	...
	void processEnded();
	void debugPreparation();
	void colorDispatcher();

	virtual void processNormal(img_t const & image);
	void processDebug(img_t const & image);
	void dispatcher(img_t const & image);
	void setResultText(QString message);
	void openImage();
